...
mod tests {
    ...
    #[test]
    fn zero_neighbour_network_time_wire_format_matches_legacy_bs() {
        let pdu = DNwrkBroadcast {
            cell_re_select_parameters: 0,
            cell_load_ca: 0,
            tetra_network_time: Some(0x59747318D7FF),
        ...
        assert_eq!(parsed.tetra_network_time, pdu.tetra_network_time);
        assert_eq!(parsed.number_of_ca_neighbour_cells, Some(0));
        assert!(parsed.neighbour_cell_information_for_ca.is_empty());
    }
}
