...
impl CcBsSubentity {
    fn send_individual_d_tx_ceased(&self, queue: &mut MessageQueue, call_id: u16, target_addr: TetraAddress, ts: u8, usage: u8) {
    ...
    fn send_individual_d_tx_granted(
        ...
        let granted_pdu = DTxGranted {
            ...
            reserved: false,
            notification_indicator: None,
            transmitting_party_type_identifier: holder_issi.map(|_| 1),
            transmitting_party_address_ssi: holder_issi.map(|ssi| ssi as u64),
        ...
        granted_pdu.to_bitbuf(&mut sdu).expect("Failed to serialize DTxGranted");
        sdu.seek(0);
        ...
        queue.push_back(Self::build_sapmsg_stealing_ul_dl(sdu, target_addr, ts, Some(usage), ul_dl));
    }

    fn notify_individual_floor_granted(&self, queue: &mut MessageQueue, call: &IndividualCall, call_id: u16, holder_issi: u32, ts: u8) {
        let dest_gssi = call.peer_addr_for(holder_issi).map(|addr| addr.ssi).unwrap_or(holder_issi);
    ...
    pub(in crate::cmce::subentities::cc_bs) fn fsm_on_u_tx_ceased(
        ...
        if let Some(call) = self.individual_calls.get(&call_id).cloned() {
            ...
            if call.is_local_participant(sender.ssi) {
                self.send_individual_d_tx_ceased(queue, call_id, sender, sender_ts, sender_usage);
            }
            ...
            {
                self.send_individual_d_tx_ceased(queue, call_id, peer_addr, peer_ts, peer_usage);
            }
