...
impl ActiveCall {
    ...
    pub(super) fn queue_tx_demand(&mut self, requester: TetraAddress, priority: u8) -> TxDemandQueueResult {
        ...
        match self.queued_tx_demand {
            Some(ref existing) if existing.requester.ssi == requester.ssi => TxDemandQueueResult::AlreadyQueuedBySameUser,
            Some(ref existing) if priority > existing.priority => {
...
pub(super) struct IndividualCall {
    ...
    pub(super) call_timeout: CallTimeout,
    /// CMCE call priority from setup. U-TX-DEMAND has a 2-bit priority, so checks saturate this to 3.
