...
pub struct MmClientProperties {
    ...
    /// several refresh attempts, MM may eventually remove it as stale.
...
mod tests {
    ...
    #[test]
    fn periodic_refresh_snooze_keeps_client_registered_without_clearing_attempts() {
        let mut mgr = MmClientMgr::new(None);
        mgr.try_register_client(2260082, true).unwrap();

        assert_eq!(mgr.set_pending_command(2260082, 60), 1);
        assert!(mgr.is_pending_command(2260082));
        assert_eq!(mgr.periodic_refresh_attempts(2260082), 1);

        mgr.snooze_periodic_refresh(2260082);
        assert!(!mgr.is_pending_command(2260082));
        assert_eq!(mgr.periodic_refresh_attempts(2260082), 1);
        assert!(mgr.collect_expired_registrations(1).is_empty());

        mgr.reset_registration_timer(2260082);
        assert_eq!(mgr.periodic_refresh_attempts(2260082), 0);
    }
}
